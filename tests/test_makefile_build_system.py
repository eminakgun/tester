...
@pytest.fixture
def makefile_config():
    return {"makefile_path": "/path/to/makefile", "make_command": "make"}
...
def test_get_available_testbenches(mock_run, makefile_system):
    mock_run.return_value = MagicMock(returncode=0, stdout=b"testbench1\ntestbench2\ntestbench3\n")
...
def test_get_available_tests(mock_run, makefile_system):
    mock_run.return_value = MagicMock(returncode=0, stdout=b"test1\ntest2\ntest3\n")
...
def config_with_separate_commands():
    return {
        "makefile_path": ".",
        "make_command": "make",
        "targets": {"testbench1": {"build_command": "make build_testbench1", "run_command": "make sim_testbench1"}},
    }


@pytest.fixture
def config_with_combined_command():
    return {"makefile_path": ".", "make_command": "make", "targets": {"testbench2": {"run_command": "make sim_testbench2"}}}
...
class TestMakefileBuildSystem:
    ...
    def test_run_with_runtime_args(self, mock_run, config_with_separate_commands):
        ...
        options = {"runtime_args": ["+UVM_TESTNAME=test1", "+TIMEOUT=1000"], "seed": 12345, "verbosity": "high"}
...
def test_build_with_custom_build_command(mock_run, makefile_config):
    ...
    config["targets"] = {
        "custom_testbench": {"build_command": "make custom_build_target", "run_command": "make custom_run_target"}
    }
