    ...
    return {
        ...
        "testbenches": {
            "testbench1": {
                "toplevel": "tb_top",
                "files": [
                    {"name": "tb/tb_top.sv", "file_type": "systemVerilogSource"}
                ],
                "parameters": {
                    "TB_PARAM": "tb_value"
                },
...
def test_run_success(mock_exists, mock_makedirs, mock_import, edalize_system):
    ...
    # Call method
    result = edalize_system.run(
        "testbench1", "test1", {"seed": 12345, "verbosity": "high"}
    )
...
def test_clean_success(mock_exists, mock_rmtree, edalize_system):
    ...
    mock_rmtree.assert_called_once_with(
        os.path.join(edalize_system.work_root, "testbench1")
    )
    assert result is True
